...
router.get("/Reviews/:id", isAuthenticated, async (req, res) => {
  // Buscar as informações no banco de Reviesws

  const Reviews = await ReviewsModel.findOne({ _id: req.params.id });
...
// Update (PATCH)
router.patch("/reviews/:id", isAuthenticated, async (req, res) => {
  try {
...
router.delete("/reviews/:id", isAuthenticated, async (req, res) => {
  try {
    const result = await ReviewsModel.deleteOne({ _id: req.params.id });
